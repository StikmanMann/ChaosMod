import { PlayerBreakBlockAfterEvent, world, system, Player } from "@minecraft/server";
import { IChaosEvent } from "ChaosMod/IChaosEvent";
import { Logger } from "staticScripts/Logger";
import { GlobalVars } from "globalVars";

const typeOrDie = () => {
    const statements = ["Wii U", "Minecraft", "AliveJy is bad", "erm what the sigma", "Nintendo 3DS", "Mario", "Jay-Z", "Pneumonoultramicroscopicsilicovolcanoconiosis", "Fair Game", "I love Chaos Minecraft!! :)"]
...
const typeOrDieTrying: IChaosEvent = {
    ...
    onChaosStop: () => {
        for(const player of GlobalVars.players){
            if(!player.hasTag("isSafe")){player.kill}
        }
        for(const player of GlobalVars.players){
            if(player.hasTag("isSafe")){player.removeTag("isSafe")}
        }
    },
    onChaosTick: () => {},
  };
