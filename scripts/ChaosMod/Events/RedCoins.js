...
const findRedCoinPosition = (player) => {
    let positions = new Array(8);
    let currentIndex = 0;
    const rot = player.getRotation();
    while (currentIndex < 8) {
        const forwardVector = VectorFunctions.getForwardVectorFromRotationXY(0, rot.y + currentIndex * (360 / 8));
...
const redCoinsEnd = () => {
    for (const player of GlobalVars.players) {
        ...
        if (!hasRedCoins) {
            player.kill();
            world.sendMessage(`§4${player.nameTag}§r was killed for not collecting the 8 red coins`);
        }
    }
};
export const redCoins = {
